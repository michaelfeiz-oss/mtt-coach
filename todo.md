...
## Strategy DB Audit Fixes (2026-05-13)
- [x] DB: Set sourceStatus = 'generated_candidate' on all 54 20bb charts
- [x] DB: Set sourceStatus = 'imported_unreviewed' on 14 null-status 15bb facing-3bet charts
- [x] DB: Set sourceStatus = 'imported_unreviewed' on SB_vs_BB_3bet @ 25bb and 40bb
- [x] UI: Add source status badge to chart viewer (source_backed / generated_candidate / simplified_population / imported_unreviewed)
- [x] Trainer gate: Block generated_candidate charts from trainer questions (already enforced via sourceTruth.ts)
- [x] Fix strategy.test.ts: replace TEST_BTN_RFI with BTN_RFI + full 169-cell seed so all 8 failing tests pass
- [x] Build exportChartInventory.mjs diagnostic tool (JSON + CSV + Markdown output)
- [x] Clean up stale test chart fixtures from DB (1170001, 1170004)
- [x] Verify: 216 production charts, all 216 complete (169 cells), zero NULL sourceStatus
