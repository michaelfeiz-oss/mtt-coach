...
} from "../server/strategy/trainerEligibilityAudit";
...
const outputPath = join(outputDirectory, `full_chart_inventory_${stamp}.csv`);
writeFileSync(outputPath, toCsv(rows));

const summary = summarizeTrainerEligibilityAudit(rows);
console.log(`Wrote ${rows.length} chart inventory rows to ${outputPath}`);
